...
: "${FLEXIBLE_RPS:?Set FLEXIBLE_RPS in base units per second}"
: "${LOYAL_RPS:?Set LOYAL_RPS in base units per second}"
: "${WHALE_RPS:?Set WHALE_RPS in base units per second}"
...
cd "$PACKAGE_DIR"
"$SUI_BIN" client ptb \
  --move-call "$PACKAGE_ID::flow_staking::set_reward_rate" "<$FLOW_COIN_TYPE>" "@$FLEXIBLE_ADMIN_CAP_ID" "@$FLEXIBLE_POOL_ID" "$FLEXIBLE_RPS" "@$CLOCK_ID" \
  --move-call "$PACKAGE_ID::flow_staking::set_reward_rate" "<$FLOW_COIN_TYPE>" "@$LOYAL_ADMIN_CAP_ID" "@$LOYAL_POOL_ID" "$LOYAL_RPS" "@$CLOCK_ID" \
  --move-call "$PACKAGE_ID::flow_staking::set_reward_rate" "<$FLOW_COIN_TYPE>" "@$WHALE_ADMIN_CAP_ID" "@$WHALE_POOL_ID" "$WHALE_RPS" "@$CLOCK_ID" \
  --gas-budget 50000000 \
  --summary
